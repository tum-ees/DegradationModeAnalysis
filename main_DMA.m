...
% be done. -> Good, robust results for fitting between 0 and 100% SOC for
% OCV and DVA in middle region (e.g. 5-95% SOC)
...
% they are saved in the following order
...
s.useBlend              = true;  % Turn on blend-electrode model by default
s.gammaBlend2_init      = 0.25;  % initial guess for gamma(Blend2)
s.gammaBlend2_upperBound = 0.30;  % e.g. 0.5 if Blend2 can reach 50% share
...
s.maxBlend2Loss   = 1; % e.g. 0.6 -> limit Blend2 loss to 60% per CU
...
if strcmp(s.direction, 'charge')
    s.Blend2Data_path = "./InputData/Silicon/" + ...
        "SiReconstr_Kuecher_P45B_Anode_Lithiation_0C03.mat";
    s.Blend1Data_path = "./InputData/Graphite/Gr_Lithiation_Kuecher.mat";
    s.cathodeData_path = "./InputData/NCA/" + ...
...
elseif strcmp(s.direction, 'discharge')
    ...
    s.Blend1Data_path = "./InputData/Graphite/Gr_Delithiation_Kuecher.mat";
    s.cathodeData_path = "./InputData/NCA/P45B_Cathode_Delithiation_0C03.mat";
else
    error('Invalid direction. Must be ''charge'' or ''discharge''.');
end

% If s.useBlend is false, treat the anode as purely graphite.
if ~s.useBlend
    s.Blend2Data_path       = s.Blend1Data_path;
    s.gammaBlend2_init      = 0;
end
...
for i = idxRange
    ...
    if s.useBlend
        if s.gammaBlend2_upperBound < 0.0001
            warning(['gammaBlend2_upperBound is very low (%.4f). ' ...
                'Please check.'], s.gammaBlend2_upperBound);
        end
    else
        s.gammaBlend2_upperBound = 0;  % pure Blend1
    end
    ...
    if s.plotParamCU
        try
            plot_OCV_model_param_show(finalSolution, i, s.cellName);
    ...
    if s.plotAllAcceptedCU && ~isempty(allAcceptedSolutions)
        for idxSol = 1:numel(allAcceptedSolutions)
            ...
            if s.plotParamCU
                plot_OCV_model_param_show(thisSol, i, s.cellName);
                ax = gca;
    ...
    if ~refDataAssigned  % or if ~isfield(Data, 'CU1'), depending on naming
        if s.useBlend
            gamma_Blend2_init = finalSolution.params(5);
        else
            gamma_Blend2_init = 0;
        end
        ...
        refData = struct('gamma_Blend2_init',       gamma_Blend2_init, ...
    ...
    [LAM_Anode, LAM_Cathode, ~, LAM_Anode_Blend2, LAM_Anode_Blend1] = ...
        ...
        refData.gamma_Blend2_init, fitReverse);
    ...
    if numel(finalSolution.params) >= 6
        Inhmg_An_prev = finalSolution.params(6);
        Inhmg_Ca_prev = finalSolution.params(7);
    else
        Inhmg_An_prev = 0;
        Inhmg_Ca_prev = 0;
    end
...
gamma_Blend2_init       = refSol.params(5);
...
if s.plotDMAOverall && ~isempty(fieldnames(Data))
    plot_DMA(Data, 'useBlendElectrodeModel', s.useBlend, ...
...
% -------------- OVERALL gamma PLOT --------------
if s.plotBlend2Overall && ~isempty(fieldnames(Data)) && s.useBlend
